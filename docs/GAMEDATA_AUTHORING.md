...
`OrrunGameData.xml` is the single authored-data document for the Python authoring foundation. The stdlib-only `tools.gamedata` module provides typed records, strict cross-reference validation, atomic saves, and deterministic XML formatting.

Validate the starter file:

```powershell
python -m tools.gamedata_viewer --validate OrrunGameData.xml
```

Open the compact all-section Tkinter viewer/editor:

```powershell
python -m tools.gamedata_viewer OrrunGameData.xml
```
